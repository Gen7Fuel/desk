  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        fields: ['key', 'id', 'amount', 'txnAmount', 'description', 'payer', 'customer.id', 'customer.name', 'txnCurrency', 'paymentMethod', 'documentNumber', 'dimensions.location.name', 'audit.createdDateTime'],
        filters: [
