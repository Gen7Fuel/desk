  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        fields: ['key', 'id', 'amount', 'txnAmount', 'description', 'currency.iso4217Code', 'deposit.payer', 'deposit.customer.id', 'deposit.customer.name', 'deposit.paymentMethod', 'deposit.documentNumber', 'dimensions.location.name', 'audit.createdDateTime'],
        filters: [
