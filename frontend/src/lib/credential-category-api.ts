...
export async function getCredentialCategories(): Promise<
  Array<CredentialCategory>
> {
...
export async function createCredentialCategory(data: {
  name: string
...
export async function updateCredentialCategory(
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => ({}))
    throw new Error(
      (body as { error?: string }).error || 'Failed to update category',
    )
  }
  return res.json()
}
